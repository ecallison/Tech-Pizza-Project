	...
	function getReceipt() {
		...
		for (var i = 0; i < veggieArray.length; i++) {
			...
			// ACCOUNTING FOR $1 COST PER ADDITIONAL VEGGIE - FIRST VEGGIE FREE
			if(customerVeggie.length > 1) {
				totalCost = totalCost + (customerVeggie.length - 1);
				veggieCost = customerVeggie.length - 1;
			} else if(customerVeggie.length <= 1) {
				totalCost = totalCost + 0;
		...
		for (var j = 0; j < meatArray.length; j++) {
			...
			// ACCOUNTING FOR $1 COST PER ADDITIONAL MEAT - FIRST MEAT FREE
			if(customerMeat.length > 1) {
				totalCost = totalCost + (customerMeat.length - 1);
				meatCost = customerMeat.length - 1;
			} else if(customerMeat.length <= 1) {
				totalCost = totalCost + 0;
				meatCost = meatCost + 0;
